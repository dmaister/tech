...
TechiesHUD.Locale = {
	...
	["forceStaffDelay"] = {
		["english"] = "Force stuff delay",
		["russian"] = "Задержка перед применением force stuff"
	},
	["forceStaffLine"] = {
...
-- Функция расчета нанесенного урона и щита после детонации
...
-- Huskar Passive 7.20 не дает маг сопротивление
...
-- Функция расчета количества бочек для убийства
...
-- Хускар больше не имеет маг резиста от пассивки
--		if NPC.HasAbility(Unit, "huskar_berserkers_blood") then -- maximum_resistance
--			local resist = (1 - (Hp - Entity.GetMaxHealth(Unit) * 0.1) / (Entity.GetMaxHealth(Unit) * 0.9)) * (Ability.GetLevelSpecialValueFor(NPC.GetAbility(Unit, "huskar_berserkers_blood"), "maximum_resistance") / 100)
--			if resist > Ability.GetLevelSpecialValueFor(NPC.GetAbility(Unit, "huskar_berserkers_blood"), "maximum_resistance") / 100 then
--				resist = Ability.GetLevelSpecialValueFor(NPC.GetAbility(Unit, "huskar_berserkers_blood"), "maximum_resistance") / 100
...
--		end
	...
	local last = 0 -- знак в числе после запятой (например, 4,3 бочки)
...
function TechiesHUD.DrawCircle(UnitPos, radius, degree)
	local x, y, visible = Renderer.WorldToScreen(UnitPos + Vector(0, radius, 0))
		local w, h = Renderer.GetScreenSize()
		if x > 0 and x < w + 100 and y > 0 and y < h + 100 then
			visible = 1
		else
			visible = 0
		end
	...
	if visible == 1 then
		for angle = 0, 360 / degree do
			...
			Renderer.SetDrawColor(0, 255, 0, 255) -- Цвет кругов (стоит зеленый)
...
function TechiesHUD.UpdateGUISettings()
	...
	optionForceDelay = GUI.Get(TechiesHUD.Identity .. "optionForceDelay")
...
function TechiesHUD.OnDraw()
	...
	if not GUI.Exist(TechiesHUD.Identity) then
		...
		GUI.AddMenuItem(TechiesHUD.Identity, TechiesHUD.Identity .. "optionForceDelay", TechiesHUD.Locale["forceStaffDelay"], GUI.MenuType.Slider, 0, 500, 500)
	...
	if optionShowEnemyRemotes then -- Если работает функция показа вражеских мин
		for i, pos in pairs(enemy_mines_pos) do
			if GameRules.GetGameTime() - pos.time > 600 then
				table.remove(enemy_mines_pos, i)
			end
			Renderer.SetDrawColor(255, 0, 0, 255) -- цвет вражеских мин (стоит красный)
			...
			if pos.ent and not Wrap.EIsAlive(pos.ent) then
				table.remove(enemy_mines_pos, i)
			end
		end
	end
	...
	local blast_damage = Ability.GetLevelSpecialValueFor(blast, "damage") + Ability.GetLevel(NPC.GetAbilityByIndex(myHero, 8)) * 300 -- Уровень скилла + талант (если есть)

	local magicalDamageMul = 1 + Hero.GetIntellectTotal(myHero) * (7/100) / 100 + 0.1 *(NPC.HasItem(myHero, "item_kaya", 1) and 1 or 0) + 0.12 *(NPC.HasItem(myHero, "item_yasha_and_kaya", 1) and 1 or 0)  
	--Исправлено на 7% за ед. интеллекта
	--Добавлена Яша и Кая
	...
	if Ability.IsInAbilityPhase(blast) and TechiesHUD.BlastPosition then
		if optionDrawCircleMethod then
			...
			if Entity.GetHealth(myHero) / Entity.GetMaxHealth(myHero) >= 0.5 then
				Renderer.SetDrawColor(255, 255, 255, 255) -- белый круг, если выживешь после суицида 
			else
				Renderer.SetDrawColor(255, 0, 0, 255) -- красный круг, если не выживешь 
			end
		...
		Wrap.DrawTextCentered(TechiesHUD.font, x, y, math.floor(Entity.GetHealth(myHero) - Entity.GetMaxHealth(myHero) * 0.5), 0)
	...
	for i, Unit in pairs(TechiesHUD.LandMinesList) do
		if NPC.HasModifier(Unit, "modifier_techies_land_mine") then
			...
			if not optionDrawCircleMethod then
				if optionLR and Wrap.EIsAlive(Unit) then
			...
			if GameRules.GetGameTime() - Modifier.GetCreationTime(NPC.GetModifier(Unit, "modifier_techies_land_mine")) < 1.75 then
				...
				local UnitsInRadius = Entity.GetUnitsInRadius(Unit, 400, Enum.TeamType.TEAM_ENEMY)
				if UnitsInRadius == nil then
					UnitsInRadius = {}
				end

				for j, Unit2 in pairs(UnitsInRadius) do
					IsActive = true
		...
		elseif not Wrap.EIsAlive(Unit) and TechiesHUD.ParticleList[Unit] then
	...
	for i, Unit in pairs(TechiesHUD.StasisMinesList) do
		local UnitPos = Entity.GetAbsOrigin(Unit)
		if not optionDrawCircleMethod then
			if optionSR and Wrap.EIsAlive(Unit) then
		...
		if not Wrap.EIsAlive(Unit) and TechiesHUD.ParticleList[Unit] then
			Particle.Destroy(TechiesHUD.ParticleList[Unit].particle)
					...
					if visible == 1 then
						...
						if Input.IsKeyDown(Enum.ButtonCode.KEY_LALT) and optionAltInfoDrawing then
							...
							if optionAltInfoTimeMines then
								...
								if Input.IsCursorInRect(x - math.floor(20 * optionAltInfoTimeMinesPanelSize), y, math.floor(20 * optionAltInfoTimeMinesPanelSize) + 1, math.floor(16 * optionAltInfoTimeMinesPanelSize) + 1) then
									...
									if Input.IsKeyDown(Enum.ButtonCode.MOUSE_RIGHT) then
										if ClickSave then
											...
											if TechiesHUD.ParticleList[Unit] ~= nil then
												if not state_mines[Unit] then
								...
								Wrap.DrawTextCentered(TechiesHUD.Panelfont1, x - math.floor(10 * optionAltInfoTimeMinesPanelSize), y + math.floor(8 * optionAltInfoTimeMinesPanelSize), text, 0)
								-- num min -
								...
								Renderer.SetDrawColor(255, 255, 255, 255)
								Wrap.DrawTextCentered(TechiesHUD.Panelfont1, x - math.floor(10 * optionAltInfoTimeMinesPanelSize), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) + math.floor(8 * optionAltInfoTimeMinesPanelSize), "-", 0)
								...
								Renderer.SetDrawColor(255, 255, 255, 255)
								Wrap.DrawTextCentered(TechiesHUD.Panelfont1, x - math.floor(10 * optionAltInfoTimeMinesPanelSize), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * 2 + math.floor(8 * optionAltInfoTimeMinesPanelSize), need_enemy[Unit], 0)
								...
								Renderer.SetDrawColor(255, 255, 255, 255)
								Wrap.DrawTextCentered(TechiesHUD.Panelfont1, x - math.floor(10 * optionAltInfoTimeMinesPanelSize), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * 3 + math.floor(8 * optionAltInfoTimeMinesPanelSize), "+", 0)
								...
								Renderer.SetDrawColor(255, 255, 255, 255)
								Wrap.DrawTextCentered(TechiesHUD.Panelfont1, x - math.floor(10 * optionAltInfoTimeMinesPanelSize), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * 4 + math.floor(8 * optionAltInfoTimeMinesPanelSize), "-", 0)
								...
								Renderer.SetDrawColor(255, 255, 255, 255)
								Wrap.DrawTextCentered(TechiesHUD.Panelfont1, x - math.floor(10 * optionAltInfoTimeMinesPanelSize), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * 5 + math.floor(8 * optionAltInfoTimeMinesPanelSize), radius_off[Unit], 0)
								...
								Renderer.SetDrawColor(255, 255, 255, 255)
								Wrap.DrawTextCentered(TechiesHUD.Panelfont1, x - math.floor(10 * optionAltInfoTimeMinesPanelSize), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * 6 + math.floor(8 * optionAltInfoTimeMinesPanelSize), "+", 0)
								------------------------------------
								...
								for i, oldest_mine in pairs(stack_mines) do
									...
									if Input.IsCursorInRect(x + math.floor(60 * optionAltInfoTimeMinesPanelSize), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * (i - 1), x + math.floor(80 * optionAltInfoTimeMinesPanelSize) - (x + math.floor(60 * optionAltInfoTimeMinesPanelSize)), y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * #stack_mines - (y + math.floor(16 * optionAltInfoTimeMinesPanelSize) * (#stack_mines - 1)) + 1) then
										...
										if Input.IsKeyDown(Enum.ButtonCode.MOUSE_RIGHT) then
											if ClickSave then
												Player.PrepareUnitOrders(Players.GetLocal(), Enum.UnitOrder.DOTA_UNIT_ORDER_CAST_NO_TARGET, nil, Vector(0, 0, 0), NPC.GetAbilityByIndex(oldest_mine.Unit, 0), Enum.PlayerOrderIssuer.DOTA_ORDER_ISSUER_PASSED_UNIT_ONLY, oldest_mine.Unit, 0, 0)
												ClickSave = false
											end
										else
											ClickSave = true
	...
	for i, Unit in pairs(Heroes.GetAll()) do
		if not NPC.IsIllusion(Unit) and not Entity.IsSameTeam(myHero, Unit) then
			if optionPanelInfo and remote ~= nil and Ability.GetLevel(remote) ~= 0 then
				...
				Renderer.SetDrawColor(255, 255, 255, 255)
				local x, y = TechiesHUD.GetHeroPos(Hero.GetPlayerID(Unit))
				Wrap.DrawTextCenteredY(TechiesHUD.HUDfont, x + 2, y + math.floor(6 * TechiesHUD.ScreenScale), math.ceil(Hp * 10) / 10, 0)
				Wrap.DrawTextCenteredY(TechiesHUD.HUDfont, x + 2, y + math.floor((6 + 14) * TechiesHUD.ScreenScale), math.ceil(Hp_all * 10) / 10, 0)
				if NPC.HasItem(Unit, "item_gem", 1) then
					Wrap.DrawTextCenteredY(TechiesHUD.HUDfont, x + TechiesHUD.GetSize() - math.floor(15 * TechiesHUD.ScreenScale), y + math.floor(6 * TechiesHUD.ScreenScale), "G", 0)
				end
				if NPC.HasItem(Unit, "item_ward_sentry", 1) or NPC.HasItem(Unit, "item_ward_sentry", 0) or NPC.HasItem(Unit, "item_ward_dispenser", 1) or NPC.HasItem(Unit, "item_ward_dispenser", 0) then
					Wrap.DrawTextCenteredY(TechiesHUD.HUDfont, x + TechiesHUD.GetSize() - math.floor(15 * TechiesHUD.ScreenScale), y + math.floor((6 + 14) * TechiesHUD.ScreenScale), "S", 0)
				end
			end
			if not Entity.IsDormant(Unit) and Wrap.EIsAlive(Unit) then
				local UnitPos = Entity.GetAbsOrigin(Unit)
				...
				if optionForceDrawLine and force ~= nil then -- force stuff line
					...
					if visible == 1 then
						...
						local w, h = Renderer.GetScreenSize()
						if x3 > 0 and x3 < h + 100 and y3 > 0 and y3 < w + 100 then
							visible = 1
						else
							visible = 0
						end
...
function TechiesHUD.OnUpdate()
	...
	local remote_damage = Ability.GetLevelSpecialValueFor(remote, "damage")
	local force = NPC.GetItem(myHero, "item_force_staff", 1)
	--if TechiesHUD.UndefEntity ~= nil and #TechiesHUD.UndefEntity ~= 0 then
		for i, Unit in pairs(TechiesHUD.UndefEntity) do
			if Wrap.EIsNPC(Unit) then
	...
	if optionDetonate then -- auto detonate
		...
		for i, Unit in pairs(TechiesHUD.RemoteMinesList) do
			local UnitPos = Entity.GetAbsOrigin(Unit)
			local heroes_in_radius = Entity.GetHeroesInRadius(Unit, 401 - (radius_off[Unit] and 455 - radius_off[Unit] or 0), Enum.TeamType.TEAM_ENEMY)
			if heroes_in_radius == nil then
				heroes_in_radius = {}
			end
			...
			for i, Unit in pairs(TechiesHUD.RemoteMinesList) do
				...
				then
					...
					if need_enemy[Unit] and num_enemy >= need_enemy[Unit] and check_enemy > 0 then
						if not Input.IsKeyDown(Enum.ButtonCode.KEY_G) then
							
							-- Взорвать одну бочку						
							--Player.PrepareUnitOrders(Players.GetLocal(), Enum.UnitOrder.DOTA_UNIT_ORDER_CAST_NO_TARGET, nil, Vector(0, 0, 0), NPC.GetAbilityByIndex(Unit, 0), Enum.PlayerOrderIssuer.DOTA_ORDER_ISSUER_PASSED_UNIT_ONLY, Unit, 0, 0)
							-- Взорвать все в радиусе
							if GameRules.GetGameTime() - time_from_detonate > 3 then
							Player.PrepareUnitOrders(Players.GetLocal(), Enum.UnitOrder.DOTA_UNIT_ORDER_CAST_POSITION, nil, Vector(UnitPos:GetX(), UnitPos:GetY(), UnitPos:GetZ()), NPC.GetAbilityByIndex(myHero, 3), Enum.PlayerOrderIssuer.DOTA_ORDER_ISSUER_CURRENT_UNIT_ONLY, myHero, 0, 0)
							time_from_detonate = GameRules.GetGameTime()
							end
							--
							--Проверить оставшеся хп
							for j, v in pairs(heroes_in_radius) do
					...
					if optionForce then -- auto force stuff
						if force ~= nil then
							...
							and force_remote_sum_damage * NPC.GetMagicalArmorDamageMultiplier(Unit) > Entity.GetHealth(Unit) and GameRules.GetGameTime() - forc_time > 0.5 then
								...
								elseif Ability.GetCooldownTimeLeft(force) == 0 and GameRules.GetGameTime() - force_direction[i] > optionForceDelay / 1000 then
...
function TechiesHUD.OnPrepareUnitOrders(orders)
	...
	or NPC.GetUnitName(orders.target) == "npc_dota_techies_remote_mine") then
		if orders.npc == 0 then
			orders.npc = Heroes.GetLocal()
		end
		Player.PrepareUnitOrders(orders.player, Enum.UnitOrder.DOTA_UNIT_ORDER_MOVE_TO_POSITION, nil, Entity.GetAbsOrigin(orders.target), nil, orders.orderIssuer, orders.npc, orders.queue, orders.showEffects)
		return false
	end
	...
	if optionFDFailSwitch and Ability.GetName(orders.ability) == "techies_focused_detonate" then
		if Input.IsKeyDown(Enum.ButtonCode.KEY_LALT) then
			...
			Player.PrepareUnitOrders(orders.player, Enum.UnitOrder.DOTA_UNIT_ORDER_CAST_POSITION, nil, Input.GetWorldCursorPos(), orders.ability, orders.orderIssuer, orders.npc, orders.queue, orders.showEffects)
			return false
		end
		local visage_stack = {}
		local templar_stack = {}
		...
		for i, Unit in pairs(TechiesHUD.RemoteMinesList) do
			...
			then
				local num_enemy = 0
				local HeroesInRadius = Entity.GetHeroesInRadius(Unit, 425 - 24, Enum.TeamType.TEAM_ENEMY)
				if HeroesInRadius == nil then
					HeroesInRadius = {}
				end
				for j, v in pairs(HeroesInRadius) do
					if Wrap.EIsAlive(v)
